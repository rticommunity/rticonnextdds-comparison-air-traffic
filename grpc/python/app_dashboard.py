...
"""
ATC Web Dashboard (gRPC) — Flask + SSE + Leaflet.js real-time map.

Pure gRPC client: discovers all ATC services via Zeroconf and subscribes
to their server-streaming RPCs.  Serves a full-screen dark Leaflet map
with animated aircraft icons, flight trails, airspace boundaries,
weather cells, alerts, and a collapsible data panel.

Run with:
    python app_dashboard.py --config ../../air_traffic_scenario.json [--port 8050]
"""
...
# ── Scenario config (loaded in main) ───────────────────────────────────────

_centers_js = "[]"
_tracons_js = "[]"
_airports_js = "{}"

# ── Shared state (written by gRPC threads, read by SSE) ────────────────────
...
def _cleanup_spawned():
    with _spawned_lock:
        for _, proc in _spawned_procs.items():
            try:
                proc.terminate()
            except OSError:
                pass
...
def position_dict(s):
    return {
        ...
        "phase": pb.FlightPhase.Name(s.flight_phase),
        "origin": s.origin_airport, "dest": s.destination_airport,
        "fuel_pct": int(s.fuel_level_percent),
        "nav_status": pb.NavStatus.Name(s.nav_status) if s.nav_status else "NAV_NORMAL",
...
def weather_dict(s):
    return {
        ...
        "condition": pb.WeatherCondition.Name(s.conditions),
        "wind": f"{s.wind.direction_degrees:03.0f}\u00b0/{s.wind.speed_knots:.0f}kt",
        "vis_m": int(s.visibility_meters),
        "ceiling_ft": s.ceiling_feet,
...
def runway_dict(s):
    return {
        ...
        "status": pb.RunwayOperationalStatus.Name(s.status),
...
def flightplan_dict(s):
    wpts = [
        {"name": w.name, "lat": round(w.position.latitude, 4),
    ...
    return {
        ...
        "to": s.arrival_airport, "status": pb.FlightPlanStatus.Name(s.status),
        "waypoints": wpts,
    }


def alert_dict(s):
    ts_ms = int(s.timestamp.seconds * 1000 + s.timestamp.nanos // 1_000_000) if s.HasField("timestamp") else 0
    return {
        "alert_id": s.alert_id, "type": pb.AlertType.Name(s.alert_type),
        "severity": pb.AlertSeverity.Name(s.severity),
...
def handoff_dict(s):
    ts_ms = int(s.initiated_at.seconds * 1000 + s.initiated_at.nanos // 1_000_000) if s.HasField("initiated_at") else 0
    return {
        "handoff_id": s.handoff_id, "tail_number": s.tail_number,
        "from": s.from_controller_id, "to": s.to_controller_id,
        "status": pb.HandoffStatus.Name(s.status),
        "from_facility": pb.FacilityType.Name(s.from_facility_type) if s.from_facility_type else "",
        "to_facility": pb.FacilityType.Name(s.to_facility_type) if s.to_facility_type else "",
        "ts": ts_ms,
    }
...
def instruction_dict(s):
    return {
        ...
        "type": pb.InstructionType.Name(s.instruction_type),
    }
...
def ack_dict(s):
    return {
        ...
        "status": pb.AcknowledgmentStatus.Name(s.status),
    }
...
def tracking_dict(s):
    return {
        ...
        "facility_type": pb.FacilityType.Name(s.facility_type),
    }
...
def convective_cell_dict(s):
    return {
        ...
        "severity": pb.ConvectiveSeverity.Name(s.severity),
        "heading": round(s.movement_heading_deg, 1),
        "speed_kt": round(s.movement_speed_knots, 1),
    }


def facility_status_dict(s):
    return {
        "facility_id": s.facility_id,
        "facility_type": pb.FacilityType.Name(s.facility_type),
        "status": "ONLINE",
        "tracked": s.tracked_aircraft_count,
    }


# ── gRPC streaming subscriber threads ──────────────────────────────────────

class GrpcPoller:
    """Discovers gRPC services via Zeroconf and streams data into shared state."""

    def __init__(self):
        self.discovery = DiscoveryManager(
            browse_roles=["aircraft", "tower", "tracon", "center",
                          "airport", "fps", "weather"]
        )
        self._known_aircraft: set[str] = set()
        self._known_airports: set[str] = set()
        self._known_towers: set[str] = set()
        self._known_tracons: set[str] = set()
        self._known_centers: set[str] = set()
        self._fps_connected = False
        self._wx_connected = False
        # Track online services for facility status liveliness
        self._online_lock = threading.Lock()
        self._online_services: dict[str, float] = {}  # facility_id → last_seen time

    def start(self):
        threading.Thread(target=self._discover_loop, daemon=True).start()
        threading.Thread(target=self._check_offline_loop, daemon=True).start()

    def _discover_loop(self):
        while not shutdown_event.is_set():
            self._discover_role("aircraft", self._known_aircraft, self._stream_aircraft)
            self._discover_role("airport", self._known_airports, self._stream_airport)
            self._discover_role("tower", self._known_towers, self._stream_tower)
            self._discover_role("tracon", self._known_tracons, self._stream_tracon)
            self._discover_role("center", self._known_centers, self._stream_center)
            # FPS (singleton)
            if not self._fps_connected:
                ep = self.discovery.get_endpoint("fps", "fps")
                if ep:
                    self._fps_connected = True
                    threading.Thread(
                        target=self._stream_fps, args=(ep[0], ep[1]), daemon=True
                    ).start()
            # Weather (singleton)
            if not self._wx_connected:
                ep = self.discovery.get_endpoint("weather", "weather")
                if ep:
                    self._wx_connected = True
                    threading.Thread(
                        target=self._stream_weather, args=(ep[0], ep[1]), daemon=True
                    ).start()
            shutdown_event.wait(2.0)

    def _discover_role(self, role, known, handler):
        services = self.discovery.get_services(role)
        for sname, (host, port, _) in services.items():
            if sname not in known:
                known.add(sname)
                threading.Thread(
                    target=handler, args=(host, port, sname), daemon=True
                ).start()

    # ── Aircraft streams ──────────────────────────────────────────────

    def _stream_aircraft(self, host, port, sname):
        try:
            channel = grpc.insecure_channel(f"{host}:{port}")
            stub = pb_grpc.AircraftServiceStub(channel)
            # Positions
            threading.Thread(
                target=self._stream_positions, args=(stub,), daemon=True
            ).start()
            # Acknowledgments
            for ack in stub.StreamAcknowledgments(pb.EmptyFilter(), timeout=600):
                if shutdown_event.is_set():
                    break
                with state_lock:
                    state["counters"]["PilotAcknowledgment"] += 1
                    state["acks"].append(ack_dict(ack))
            channel.close()
        except grpc.RpcError:
            self._known_aircraft.discard(sname)

    def _stream_positions(self, stub):
        try:
            for pos in stub.StreamPositions(pb.EmptyFilter(), timeout=600):
                if shutdown_event.is_set():
                    break
                d = position_dict(pos)
                with state_lock:
                    state["counters"]["AircraftPosition"] += 1
                    state["positions"][pos.tail_number] = d
                    state["trails"][pos.tail_number].append([d["lat"], d["lon"]])
        except grpc.RpcError:
            pass

    # ── Airport streams ───────────────────────────────────────────────

    def _stream_airport(self, host, port, sname):
        try:
            channel = grpc.insecure_channel(f"{host}:{port}")
            ...
            for rwy in stub.StreamRunwayStatus(pb.EmptyFilter(), timeout=600):
                if shutdown_event.is_set():
                    break
                with state_lock:
                    state["counters"]["RunwayStatus"] += 1
                    key = f"{rwy.airport_code}/{rwy.runway_id}"
                    state["runways"][key] = runway_dict(rwy)
            channel.close()
        except grpc.RpcError:
            self._known_airports.discard(sname)

    def _stream_airport_weather(self, stub):
        try:
            for wx in stub.StreamWeatherReports(pb.EmptyFilter(), timeout=600):
                if shutdown_event.is_set():
                    break
                with state_lock:
                    state["counters"]["WeatherReport"] += 1
                    state["weather"][wx.airport_code] = weather_dict(wx)
        except grpc.RpcError:
            pass

    # ── Tower streams ─────────────────────────────────────────────────

    def _stream_tower(self, host, port, sname):
        try:
            channel = grpc.insecure_channel(f"{host}:{port}")
            stub = pb_grpc.TowerServiceStub(channel)
            threads = [
                threading.Thread(target=self._stream_generic, daemon=True,
                    args=(lambda: stub.StreamInstructions(pb.ControllerInstructionFilter(), timeout=600),
                          "ControllerInstruction", lambda s: instruction_dict(s))),
                threading.Thread(target=self._stream_generic, daemon=True,
                    args=(lambda: stub.StreamHandoffs(pb.HandoffFilter(), timeout=600),
                          "Handoff", lambda s: handoff_dict(s))),
                threading.Thread(target=self._stream_generic, daemon=True,
                    args=(lambda: stub.StreamAlerts(pb.EmptyFilter(), timeout=600),
                          "Alert", lambda s: alert_dict(s))),
                threading.Thread(target=self._stream_generic, daemon=True,
                    args=(lambda: stub.StreamTracking(pb.EmptyFilter(), timeout=600),
                          "AircraftTracking", lambda s: tracking_dict(s))),
                threading.Thread(target=self._stream_generic, daemon=True,
                    args=(lambda: stub.StreamFacilityStatus(pb.EmptyFilter(), timeout=600),
                          "FacilityStatus", lambda s: facility_status_dict(s))),
            ]
            for t in threads:
                t.start()
            for t in threads:
            ...
            self._known_towers.discard(sname)

    # ── TRACON streams ────────────────────────────────────────────────

    def _stream_tracon(self, host, port, sname):
        try:
            channel = grpc.insecure_channel(f"{host}:{port}")
            ...
            self._known_tracons.discard(sname)

    # ── Center streams ────────────────────────────────────────────────

    def _stream_center(self, host, port, sname):
        try:
            channel = grpc.insecure_channel(f"{host}:{port}")
            stub = pb_grpc.CenterServiceStub(channel)
            threads = [
                threading.Thread(target=self._stream_generic, daemon=True,
                    args=(lambda: stub.StreamInstructions(pb.ControllerInstructionFilter(), timeout=600),
                          "ControllerInstruction", lambda s: instruction_dict(s))),
            ...
            self._known_centers.discard(sname)

    # ── FlightPlan stream ─────────────────────────────────────────────

    def _stream_fps(self, host, port):
        try:
            channel = grpc.insecure_channel(f"{host}:{port}")
            stub = pb_grpc.FlightPlanServiceStub(channel)
            for plan in stub.StreamFlightPlans(pb.EmptyFilter(), timeout=600):
                if shutdown_event.is_set():
                    break
                with state_lock:
                    state["counters"]["FlightPlan"] += 1
                    state["flight_plans"][plan.flight_plan_id] = flightplan_dict(plan)
            channel.close()
        except grpc.RpcError:
            self._fps_connected = False

    # ── Weather stream (convective cells) ─────────────────────────────

    def _stream_weather(self, host, port):
        try:
            channel = grpc.insecure_channel(f"{host}:{port}")
            stub = pb_grpc.WeatherServiceStub(channel)
            for cell in stub.StreamConvectiveCells(pb.EmptyFilter(), timeout=600):
                if shutdown_event.is_set():
                    break
                with state_lock:
                    state["counters"]["ConvectiveCell"] += 1
                    state["convective_cells"][cell.cell_id] = convective_cell_dict(cell)
            channel.close()
        except grpc.RpcError:
            self._wx_connected = False
    ...
    def _stream_generic(self, stream_factory, topic_name, dict_fn):
        try:
            for sample in stream_factory():
                if shutdown_event.is_set():
                    break
                d = dict_fn(sample)
                with state_lock:
                    ...
                    if topic_name == "Alert":
                        state["alerts"].append(d)
                    elif topic_name == "Handoff":
                        state["handoffs"].append(d)
                        state["handoff_log"].append(d)
                        if d["status"] == "ACCEPTED" and d.get("to_facility") == "CENTER":
                            state["pending_pulses"].append(
                                d["to"].replace("CTR-", "", 1)
                            )
                    elif topic_name == "ControllerInstruction":
                        state["instructions"].append(d)
                    elif topic_name == "AircraftTracking":
                        state["tracking"][d["tail_number"]] = d
                    elif topic_name == "FacilityStatus":
                        fid = d["facility_id"]
                        state["facility_status"][fid] = d
                        with self._online_lock:
                            self._online_services[fid] = time.time()
        except grpc.RpcError:
            pass
    ...
    def _check_offline_loop(self):
        """Mark facilities as OFFLINE if no update in 10 seconds."""
        while not shutdown_event.is_set():
            now = time.time()
            with self._online_lock:
                stale = [fid for fid, ts in self._online_services.items()
                         if now - ts > 10.0]
            if stale:
                with state_lock:
                    for fid in stale:
                        if fid in state["facility_status"]:
...
def _snapshot():
    with state_lock:
        ...
        facility_status = list(state["facility_status"].values())
        type_order = {"CENTER": 0, "TRACON": 1, "TOWER": 2, "NATIONAL": 3}
        facility_status.sort(
            key=lambda f: (type_order.get(f["facility_type"], 9), f["facility_id"])
        )
...
def create_weather_cell():
    """Create a weather cell and forward it to the WeatherService via InjectCell."""
    ...
    sev_map = {"MODERATE": pb.MODERATE, "SEVERE": pb.SEVERE, "EXTREME": pb.EXTREME}
    if severity not in sev_map:
        return {"error": f"severity must be one of {list(sev_map)}"}, 400

    cell_id = make_id("WX-")

    # Forward to WeatherService so centers receive the cell
    injected_via_grpc = False
    if _grpc_poller:
        ep = _grpc_poller.discovery.get_endpoint("weather", "weather")
        if ep:
            try:
                channel = grpc.insecure_channel(f"{ep[0]}:{ep[1]}")
                stub = pb_grpc.WeatherServiceStub(channel)
                cell_proto = pb.ConvectiveCell(
                    cell_id=cell_id,
                    center_latitude=lat,
                ...
                ack = stub.InjectCell(cell_proto, timeout=5)
                injected_via_grpc = ack.accepted
                if injected_via_grpc:
                    cell_id = ack.cell_id
                channel.close()
            except Exception as e:
                log.warning("Failed to inject cell via WeatherService: %s", e)

    # Also store locally for dashboard display
    cell = {
        "cell_id": cell_id, "lat": lat, "lon": lon,
        "radius_nm": radius, "top_alt": top_alt, "base_alt": base_alt,
        "severity": severity, "heading": hdg, "speed_kt": spd,
    }
    with state_lock:
        state["convective_cells"][cell_id] = cell
...
def spawn_aircraft():
    ...
    if origin not in _AIRPORT_CODES or destination not in _AIRPORT_CODES:
        return {"error": "unknown airport code"}, 400
    ...
    return {"callsign": callsign, "origin": origin, "destination": destination,
            "pid": proc.pid}
...
def list_spawned():
    with _spawned_lock:
        result = [
            {"callsign": cs, "pid": proc.pid, "running": proc.poll() is None}
...
<title>ATC Dashboard (gRPC)</title>
...
#spawn-btn:hover, #wx-btn:hover { background: #6bc0f0; }
#spawn-btn:disabled, #wx-btn:disabled { opacity: 0.5; cursor: not-allowed; }
#spawn-msg, #wx-msg { font-size: 0.72rem; min-height: 1.2em; }
#spawn-msg.ok, #wx-msg.ok { color: var(--green); }
#spawn-msg.err, #wx-msg.err { color: var(--red); }
...
<!-- Top bar with KPIs -->
<div id="topbar">
  <div class="logo">&#9992;&#65039; <span>ATC</span> Dashboard <span style="font-size:0.6em;color:var(--dim)">(gRPC)</span></div>
  <div class="kpi-row">
...
function renderAircraft(positions, trails, tracking) {
  ...
  positions.forEach(function(ac) {
    ...
    aircraftMarkers[ac.tail_number].getPopup().setContent(
      ...
      '<span class="phase phase-' + ac.phase + '">' + ac.phase + "</span>" + (ac.gate ? ' Gate ' + ac.gate : '') + (ac.nav_status === 'NAV_WEATHER_DEVIATION' ? ' <span class="nav-wx">⚡WX DEV</span>' : '') + ctrlLine + "<br>" +
    ...
    var wxBadge = ac.nav_status === 'NAV_WEATHER_DEVIATION' ? ' <span class="nav-wx">⚡WX</span>' : '';
...
function update(d) {
  ...
  renderTable("ac-body", d.positions.map(function(ac) {
    ...
    return '<tr' + sel + ' onclick="selectAircraft(\'' + ac.tail_number + '\')"><td>' + ac.callsign + "</td>" +
           ...
           '<td><span class="phase phase-' + ac.phase + '">' + ac.phase + "</span>" + (ac.gate ? ' ' + ac.gate : '') + (ac.nav_status === 'NAV_WEATHER_DEVIATION' ? ' <span class="nav-wx">⚡WX</span>' : '') + "</td>" +
