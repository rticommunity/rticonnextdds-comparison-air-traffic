...
#
# demo_start.sh — Launch gRPC ATC demo applications individually or all at once.
#
# Usage:
...
#
# Global options:
#   --duration N   Run duration in seconds (default: from scenario config)
#
# Examples:
#   ./demo_start.sh all
...
DURATION=""
# Scenario config at repo root (shared across implementations)
...
start_flightplan() {
    ...
    "$PYTHON" "$PYTHON_DIR/app_flightplan_service.py" --config "$SCENARIO_CONFIG" --duration "$dur" &
    PIDS+=($!)
}
...
start_dashboard() {
    ...
    "$PYTHON" "$PYTHON_DIR/app_dashboard.py" --config "$SCENARIO_CONFIG" --port "$port" &
    PIDS+=($!)
}
